...
//  Variables
unsigned char ssd1306_page;
unsigned char ssd1306_column;
unsigned char rotation;

void ssd1306_start_command(unsigned char cmd)
{
...
void ssd1306_fill_screen(unsigned char b)
{
...
void ssd1306_display(unsigned char *image)
{
...
void ssd1306_set_pos(unsigned char column, unsigned char page)
{
...
void ssd1306_draw_circle(unsigned char x, unsigned char y, unsigned char radius, unsigned char color)
{
    int xp = 0, yp = radius;
    int d = 3 - (2 * radius);
    while (xp <= yp)
    {
        ssd1306_set_pixel(x + xp, y + yp, color);
        ssd1306_set_pixel(x + yp, y + xp, color);
        ssd1306_set_pixel(x - xp, y + yp, color);
        ssd1306_set_pixel(x + yp, y - xp, color);
        ssd1306_set_pixel(x - xp, y - yp, color);
        ssd1306_set_pixel(x - yp, y - xp, color);
...
void ssd1306_goto_row(unsigned char page)
{
...
void ssd1306_display_decimal_byte(unsigned long num, unsigned char leading_zeros)
{
    uart_send_string("ssd1306_display_decimal_byte\n");

    unsigned long devisor = 1000000000;
    unsigned char c;
    while (devisor > 1)
    {
        c = num / devisor;
...
void ssd1306_flip_horizontal(unsigned char mode)
{
    ssd1306_send_command(mode ? SSD1306_SEGREMAPINV : SSD1306_SEGREMAPNORMAL);
}

void ssd1306_flip_vertical(unsigned char mode)
{
